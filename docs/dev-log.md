...
### 迭代：探测与凭据写入分离（方案一，用户讨论后确认）
- **用户反馈**：探测即覆盖本地凭据（cmdkey 提前写入），且已有会话时探测
  复用旧凭据，无法验证其他凭据；探测失败的"回滚"实际是删除（cmdkey
  读不回旧密码），原凭据丢失。
- **新设计**：
  - 探测阶段不写凭据管理器：
    ①本机已有会话/凭据可直接枚举（零副作用，日志注明"使用本机已有凭据；输入的凭据将在映射时验证"）；
    ②否则用输入凭据建临时会话（net use IPC$，不落盘）枚举后拆除；
    ③Samba NAS 拒绝显式 IPC$（错误 67）时 cmdkey 兜底，失败回滚。
  - 凭据只在点"映射选中共享"时写入：MapSharesAsync 增加credsByServer 参数（Tab1 按服务器区分 CORP1/corp2 账号，密码在点击时由员工号重新计算，不在内存驻留），写入后清旧会话再映射；某服务器映射全部失败时回滚刚保存的凭据并提示检查用户名密码。
- **端到端验证**（本机已有真实 NAS 会话）：UI 自动化输入假凭据探测真实NAS → 探测成功（复用已有凭据）、共享列表正确显示（Finance/HR_ Admin/SG-NAS + 盘符徽章）、**cmdkey 列表前后完全一致（零写入）**。
- 测试脚本踩坑：SendKeys 被微软拼音输入法拦截导致输入串行→ 改用 SendInput KEYEVENTF_UNICODE 注入（绕过 IME）；第一次实现 INPUT 结构体布局错误（x64 需 40 字节）导致注入静默失败，补上 MOUSEINPUT 联合后正常。

### 迭代：development-prompt.md 更新为 v4
- "正确的探测流程"章节重写为"探测与凭据写入分离"（方案一落地后的设计）：
  探测三级策略（已有会话零副作用 → 临时会话 → cmdkey 兜底+回滚）、
  映射阶段才写凭据（按服务器区分域账号、全败回滚）、cmdkey 不可读回
  旧密码的约束说明、共享过滤与错误诊断并入本节；
- 验证章节补充：自动化输入用 SendInput + KEYEVENTF_UNICODE 绕过中文 IME。
